...
| Phase14 | MISRA対応 | 着手中。cppcheckのMISRA C:2012アドオンで検出した12ルールを、判断の軽いものから段階的に対処する方針で進行中。機械的で判断が軽いルール（5.9識別子重複・8.9ブロックスコープ・10.4/10.8本質的な型・15.6複合文・15.7終端else、計6ルール）に加え、件数は多いが判断基準が単純なルール（17.7戻り値未使用、26件）を解消済み。17.7のうち大半は`(void)`キャストで対応したが、`persist.c`の保存処理（`fputs`/`fclose`の戻り値を見ずに常に成功扱いしていた箇所）は実質的な修正が必要と判断し、書き込み失敗を正しく検出するよう修正した上で、`/dev/full`を使った書き込み失敗時の自動テストを追加した。残る、既存の設計と衝突しうるルール（21.6/21.10標準ライブラリ制限・12.1括弧の明示・18.4ポインタ演算・15.5単一出口）は未着手。通常動作の出力は変更していない |
