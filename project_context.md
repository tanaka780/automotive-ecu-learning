...
- Phase: Phase1〜10は完了（詳細はstudy_plan.md参照）
- 実装済み: センサシミュレーション、統計、アラート、センサ状態、固定幅整数型、DTC（記録・状態区分・フリーズフレーム）、logger、イグニッション状態管理、DTC永続化、診断コマンド（`clear`によるDTC全体クリア、`clear <センサ名>`によるセンサ単位クリア、想定外入力の理由別通知、イグニッションOFF時のみの受付制限）、閾値の設定ファイル化（`config.c`。`alert.h`の3閾値+`status.h`の6閾値（計9個）をKEY=VALUE形式のファイルから読み込み、`alert_check`/`status_check`に`const ConfigData *`として渡す）、ログレベル制御（`logger.c`の`log_print_leveled`/`logger_set_level`。`config.txt`の`LOG_LEVEL`（0=INFO/1=WARNING/2=ERROR）で実行時の表示閾値を切り替え、既存のログ呼び出しをほぼ`log_print_leveled`に置き換え済み。`config.c`自身の読み込み結果ログ2箇所のみ、表示閾値が確定する前に呼ばれるため`log_print_tagged`のまま対象外とした）（詳細はモジュール構成表・study_plan.md参照）
- シナリオ定義: docs/scenarios.mdに、正常走行／故障発生／診断コマンド（DTCクリア）／電源再投入／設定ファイル異常時のフェイルセーフ（リンプホームモード）／通信故障（将来項目）の6シナリオを「目的／流れ／期待結果」の形式で整理済み（Phase10）。固定値によるシナリオ再現の仕組みはPhase11で構築予定
