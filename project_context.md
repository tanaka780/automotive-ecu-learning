...
- Phase: Phase1〜14は完了（Phase14はMISRA対応。cppcheck `--addon=misra`で検出した12ルールのうち、機械的で判断が軽いルール（5.9・8.9・10.4・10.8・15.6・15.7、計6ルール）、件数は多いが判断基準が単純なルール（17.7戻り値未使用、26件）、既存設計への影響が小さいルール（12.1演算子優先順位の明示・18.4ポインタ演算、計5件）を適用した。17.7の大半は`(void)`キャストで対応したが、`persist.c`の保存処理（`fputs`/`fclose`の戻り値を見ずに常に成功扱いしていた箇所）は`(void)`キャストでは済まない実質的な修正と判断し、書き込み失敗を正しく検出するよう修正した（`/dev/full`を使った自動テストも追加）。残る21.6/21.10（標準入出力・time.h使用制限）・15.5（単一出口）は不採用と判断した：前者はPC上のシミュレータという設計前提そのもの（printf/fopen/timeを使う）と衝突しており、対応するならtarget向け実装を別途用意する将来のRenode等によるtarget化構想の中で扱う話であって今のPC版コードから排除するものではないため、後者はガード節（早期return）による可読性重視の設計と衝突し、41箇所の書き換えはCLAUDE.mdが避けたい大規模リファクタリングに該当するため。詳細はstudy_plan.md参照）
- 実装済み: センサシミュレーション、統計、アラート、センサ状態、固定幅整数型、DTC（記録・状態区分・フリーズフレーム）、logger、イグニッション状態管理、DTC永続化、診断コマンド（`clear`によるDTC全体クリア、`clear <センサ名>`によるセンサ単位クリア、想定外入力の理由別通知、イグニッションOFF時のみの受付制限）、閾値の設定ファイル化（`config.c`。`alert.h`の3閾値+`status.h`の6閾値（計9個）をKEY=VALUE形式のファイルから読み込み、`alert_check`/`status_check`に`const ConfigData *`として渡す）、ログレベル制御（`logger.c`の`log_print_leveled`/`logger_set_level`。`config.txt`の`LOG_LEVEL`（0=INFO/1=WARNING/2=ERROR）で実行時の表示閾値を切り替え、既存のログ呼び出しをほぼ`log_print_leveled`に置き換え済み。`config.c`自身の読み込み結果ログ2箇所のみ、表示閾値が確定する前に呼ばれるため`log_print_tagged`のまま対象外とした）、固定値注入（`fixture.c`。`fixture.txt`をKEY=VALUE形式（`MODE=FIXED/RANDOM`、`SPEED`/`RPM`/`TEMP`）で読み込み、`MODE=FIXED`なら`sensor_update`のランダム生成をセンサ固定値に差し替える。ファイル無し／`MODE=RANDOM`なら従来通りランダム動作のまま）、入力値域チェック（`validate.c`。`SensorId`をインデックスにした値域テーブル（speed 0〜120／rpm 0〜6000／temp 25〜100）とLOG_LEVEL用の範囲チェック（0〜2）を提供し、config.c・fixture.cがパース直後の値を反映する前に呼び出す。範囲外の値はそのキーだけ無視される。Phase12で追加、`test/test_validate.c`で境界値まで自動テスト済み。Phase14で`validate_in_range`/`validate_log_level`の比較式に括弧を追加（MISRA 12.1））（詳細はモジュール構成表・study_plan.md参照）
...
- テスト（Phase13よりUnity形式に統一）: `test/test_diag.c`（固定値データでdiag.cの動作を確認。`status_check`の境界値確認は直接呼び出し。非デフォルト`ConfigData`での分類変化も確認）、`test/test_persist.c`（固定値データ・意図的に壊したデータでpersist.cの正常系・異常系を確認。加えて`/dev/full`を使い、書き込み自体が失敗した場合に保存失敗を正しく検出することも確認）、`test/test_stats.c`（固定値データでstats.cのmin/max/sum/countの更新を確認）、`test/test_alert.c`（標準出力キャプチャでalert.cの警告出力を確認。`alert_check`の直接呼び出し。非デフォルト`ConfigData`での警告有無の変化も確認）、`test/test_ignition.c`（標準出力キャプチャでignition.cの遷移イベント出力を確認）、`test/test_cmd.c`（固定値データで`diag_clear`・`diag_clear_sensor`・`cmd_dispatch`（全体クリア／センサ単位クリア／不正なセンサ名／余分なトークン／空白のみ）の動作を確認。標準入力を扱う`cmd_read_line`は対象外）、`test/test_config.c`（`config_load`のファイルパースを確認。正常系：全9キーが反映される、異常系：未知のキー・値欠落・数値以外の行は無視される、異常系：値域外の値は無視される、異常系：キー重複時は後勝ち）、`test/test_fixture.c`（`fixture_apply`のファイルパースを確認。正常系：`MODE=FIXED`で全キーが反映される、異常系：未知のキー・値欠落・数値以外の行は無視される・値域外の値は無視される・キー重複時は後勝ち・`MODE=RANDOM`時はセンサ値を変更しない）、`test/test_validate.c`（`validate_in_range`/`validate_log_level`の境界値を確認。speed/rpm/temp各センサの下限・上限・範囲外、不正な`SensorId`、LOG_LEVELの下限・上限・範囲外）。いずれも`make test`で実行し、Unity形式（`vendor/unity/`、`TEST_ASSERT_*`系マクロ・`setUp`/`tearDown`・`UNITY_BEGIN`/`RUN_TEST`/`UNITY_END`）で判定・集計する。サンプル投入用の`test_feed`/`test_run_sample`（`DtcRecord`前提）は`test/test_common.c`から`test_diag.c`/`test_persist.c`/`test_cmd.c`が使用し、デフォルトの`ConfigData`を返す`test_default_config()`は同じく`test_common.c`から`test_diag.c`・`test_alert.c`・`test_config.c`が使用する。`test_stats.c`・`test_ignition.c`・`test_fixture.c`・`test_validate.c`はこれらを使わないため`test_common.c`への依存が無い（`test_stats.c`はローカルのヘルパーを使う）。test_alert.c・test_ignition.c・test_config.c・test_fixture.cは標準出力キャプチャ用／ファイル書き込み用のヘルパーをそれぞれファイル内にローカルで定義している。sensor.c/logger.c/cmd.c（`cmd_read_line`のみ）/config.c（`config_print`のみ）は未テスト
...
| `persist.c` | `DtcRecord`（読み込み時のみ書く） | `DtcRecord`（保存時は読むのみ） | `DtcRecord`とテキストの相互変換、ファイルへの保存・読み込み、成功/失敗のログ表示。保存時は`fputs`/`fclose`の戻り値を見て実際の書き込み失敗を検出する（Phase14、MISRA Directive 4.7対応）。`previous`は保存対象に含めない |
...
| `test/test_persist.c` | — | `persist.c`・`diag.c`・`status.c`（読むのみ） | 固定値データと意図的に壊したデータを使い、persist.cの保存・読み込みが正常系・異常系（ファイル無し、値不足、数値以外）で期待通りかを確認する。加えて、Linux上で書き込みが必ず失敗する`/dev/full`へ保存させ、`persist_save_dtc`が書き込み失敗を正しく検出してfalseを返すことも確認する（WSL2 Ubuntu環境前提のテスト）。本番の`dtc_data.txt`とは別のテスト専用ファイル名を使う |
...
| `test/test_common.c` | — | `config.c`（`config_init`のみ読むのみ） | `test_diag.c`/`test_persist.c`/`test_cmd.c`で共通のサンプル投入ヘルパー（`test_feed`/`test_run_sample`、`DtcRecord`前提）と、`test_diag.c`・`test_alert.c`・`test_config.c`で共通のデフォルト`ConfigData`（`test_default_config`）を提供する。Phase13でテスト自体をUnity形式に統一したため、結果判定・サマリ表示（旧`test_check`/`test_summary`）の役割はUnityに置き換わった。`test_stats.c`・`test_ignition.c`・`test_fixture.c`・`test_validate.c`はこのファイルを使わない |
