...
- Phase: Phase1〜11は完了（詳細はstudy_plan.md参照）
- 実装済み: センサシミュレーション、統計、アラート、センサ状態、固定幅整数型、DTC（記録・状態区分・フリーズフレーム）、logger、イグニッション状態管理、DTC永続化、診断コマンド（`clear`によるDTC全体クリア、`clear <センサ名>`によるセンサ単位クリア、想定外入力の理由別通知、イグニッションOFF時のみの受付制限）、閾値の設定ファイル化（`config.c`。`alert.h`の3閾値+`status.h`の6閾値（計9個）をKEY=VALUE形式のファイルから読み込み、`alert_check`/`status_check`に`const ConfigData *`として渡す）、ログレベル制御（`logger.c`の`log_print_leveled`/`logger_set_level`。`config.txt`の`LOG_LEVEL`（0=INFO/1=WARNING/2=ERROR）で実行時の表示閾値を切り替え、既存のログ呼び出しをほぼ`log_print_leveled`に置き換え済み。`config.c`自身の読み込み結果ログ2箇所のみ、表示閾値が確定する前に呼ばれるため`log_print_tagged`のまま対象外とした）、固定値注入（`fixture.c`。`fixture.txt`をKEY=VALUE形式（`MODE=FIXED/RANDOM`、`SPEED`/`RPM`/`TEMP`）で読み込み、`MODE=FIXED`なら`sensor_update`のランダム生成をセンサ固定値に差し替える。ファイル無し／`MODE=RANDOM`なら従来通りランダム動作のまま）（詳細はモジュール構成表・study_plan.md参照）
...
| `config.c` | `ConfigData` | 設定ファイルの文字列、`alert.h`/`status.h`のマクロ（デフォルト値として） | 閾値9個（alert.hの3つ+status.hの6つ）とログレベル（`log_level`、デフォルト`LOG_INFO`）を初期化し（`config_init`）、`KEY=VALUE`形式の設定ファイルがあれば読み込んで上書きする（`config_load`）。ファイルが無い場合はデフォルト値のまま変更しない。読み込んだ値を1行でログ出力する（`config_print`、`log_print_leveled`を使用）。`main.c`が読み込んだ`ConfigData`を`alert_check`/`status_check`に渡すことで閾値判定に、`config.log_level`を`logger_set_level`に渡すことで表示閾値に反映する。`config_load`自身の読み込み結果ログ2箇所は、その時点でまだ表示閾値が確定していないため`log_print_tagged`のまま（レベル制御の対象外） |
| `fixture.c` | `VehicleSensorData`（`MODE=FIXED`の時のみ書く） | 固定値注入ファイルの文字列 | `fixture.txt`を`KEY=VALUE`形式（`MODE`/`SPEED`/`RPM`/`TEMP`）で読み込む（`fixture_apply`）。`MODE=FIXED`なら指定されたセンサ値を`VehicleSensorData`に反映してtrueを返す（未指定のキーは呼び出し時点の値を保持）。ファイルが無い場合・`MODE=RANDOM`の場合・`MODE`行が無い場合はfalseを返し値を変更しない。`main.c`はtrueが返れば以降`sensor_update`を呼ばない（1回の実行中は値が変化しない単一固定値のみに対応、サンプルごとの推移は対象外） |
...
| `test/test_fixture.c` | — | `fixture.c`（読むのみ） | 固定値のテキストファイルを直接書き込み（`fixture_apply`を経由せず、任意の内容のファイルを再現する）、`fixture_apply`が正常系（`MODE=FIXED`で全キーが反映される）・異常系（未知のキー・値欠落・数値以外の行は無視される、キー重複時は後勝ち、`MODE=RANDOM`時はfalseを返しセンサ値を変更しない）で期待通りに動作するかを確認する。本番の`fixture.txt`とは別のテスト専用ファイル名（`test_fixture.txt`）を使う |
