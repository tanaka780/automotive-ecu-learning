...
- Phase: Phase1〜13は完了、Phase14（MISRA対応）は着手中（cppcheck `--addon=misra`で検出した12ルールのうち、機械的で判断が軽いルール（5.9・8.9・10.4・10.8・15.6・15.7、計6ルール）と、件数は多いが判断基準が単純なルール（17.7戻り値未使用、26件）を解消済み。17.7の大半は`(void)`キャストで対応したが、`persist.c`の保存処理（`fputs`/`fclose`の戻り値を見ずに常に成功扱いしていた箇所）は`(void)`キャストでは済まない実質的な修正と判断し、書き込み失敗を正しく検出するよう修正した（`/dev/full`を使った自動テストも追加）。残る、既存の設計と衝突しうるルール（21.6/21.10・12.1・18.4・15.5）は未着手。詳細はstudy_plan.md参照）
