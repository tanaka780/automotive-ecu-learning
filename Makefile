...
TEST_DIAG_SRCS = test/test_diag.c test/test_common.c src/status.c src/diag.c src/logger.c src/config.c src/validate.c
TEST_DIAG_TARGET = test_diag

# persist.c の動作確認用テスト（ファイルI/Oの正常系・異常系、main.c は使わない）
TEST_PERSIST_SRCS = test/test_persist.c test/test_common.c src/status.c src/diag.c src/logger.c src/persist.c src/config.c src/validate.c
...
TEST_STATS_SRCS = test/test_stats.c test/test_common.c src/status.c src/diag.c src/logger.c src/stats.c src/config.c src/validate.c
TEST_STATS_TARGET = test_stats

# alert.c の動作確認用テスト（標準出力キャプチャによる警告出力の確認、main.c は使わない）
# test_common.c が status_check/diag_check を参照するため status.c/diag.c も含める
TEST_ALERT_SRCS = test/test_alert.c test/test_common.c src/status.c src/diag.c src/logger.c src/alert.c src/config.c src/validate.c
TEST_ALERT_TARGET = test_alert

# ignition.c の動作確認用テスト（標準出力キャプチャによる遷移イベント出力の確認、main.c は使わない）
# test_common.c が status_check/diag_check を参照するため status.c/diag.c も含める
TEST_IGNITION_SRCS = test/test_ignition.c test/test_common.c src/status.c src/diag.c src/logger.c src/ignition.c src/config.c src/validate.c
TEST_IGNITION_TARGET = test_ignition

# cmd.c の動作確認用テスト（diag_clear・cmd_dispatchの確認、main.c は使わない）
# test_common.c が status_check/diag_check を参照するため status.c/diag.c も含める
TEST_CMD_SRCS = test/test_cmd.c test/test_common.c src/status.c src/diag.c src/logger.c src/cmd.c src/config.c src/validate.c
TEST_CMD_TARGET = test_cmd

# config.c の動作確認用テスト（config_loadのファイルパースの正常系・異常系の確認、main.c は使わない）
# test_common.c が status_check/diag_check を参照するため status.c/diag.c も含める
TEST_CONFIG_SRCS = test/test_config.c test/test_common.c src/status.c src/diag.c src/logger.c src/config.c src/validate.c
